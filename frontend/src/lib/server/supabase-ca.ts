/**
 * Supabase Root CA certificates.
 *
 * Supabase uses its own CA ("Supabase Root 2021 CA") to sign database TLS
 * certificates.  Because this CA is not in system trust stores, node-postgres
 * needs it supplied explicitly via `ssl.ca`.
 *
 * Both the original (2021) and rotated (2025) root certificates are included
 * so connections survive key rotation.
 *
 * Source:
 *   Dashboard  → Project Settings → Database → SSL → Download Certificate
 *   https://github.com/supabase/cli  (internal/gen/types/templates/)
 */

const SUPABASE_ROOT_2021 = `-----BEGIN CERTIFICATE-----
...
const SUPABASE_ROOT_2025 = `-----BEGIN CERTIFICATE-----
...
4LRCXSlQsDgQ1LZ4KHsl4cq7K51N6FOXQBLB5q4lMKhs0VUhCT8Pdsj1E6gZzn4B
Ih1gBnVcRe2uOBDq7jQ3B3kBCIFhKPsfFsdeyiblwGMsKT4NaKhbRiMMEvHRPSWG
kHP4VPSJyB/B7fU=
-----END CERTIFICATE-----`;

/**
 * Combined PEM bundle trusted when connecting to Supabase Postgres / Supavisor.
 * Pass as `ssl.ca` in the pg Pool config.
 */
export const SUPABASE_ROOT_CERTS = [SUPABASE_ROOT_2021, SUPABASE_ROOT_2025].join(
  "\n"
);

...
app.add_middleware(
    ...
    allow_origins=[
        ...
        *(["http://localhost:3000"] if "localhost" in settings.FRONTEND_URL else []),
    ],
    allow_origin_regex=r"https://agent-meet-.*-matanraks-projects\.vercel\.app",
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Request logging (added after CORS, runs before rate limiting)
app.add_middleware(RequestLoggingMiddleware)

# Rate limiting (added last so it runs as the outermost middleware)
...
async def pool_exhaustion_handler(request: Request, exc: asyncpg.exceptions.InterfaceError):
    """Return 503 when the DB connection pool is exhausted."""
    logger.warning("DB pool exhausted: %s %s", request.method, request.url.path)
    return JSONResponse(
        status_code=503,
        content={
            "error": "service_busy",
            "message": "Server is under heavy load. Please retry in a few seconds.",
        },
    )
...
async def timeout_handler(request: Request, exc: asyncio.TimeoutError):
    """Return 503 on DB query timeouts."""
    logger.warning("DB timeout: %s %s", request.method, request.url.path)
    return JSONResponse(
        status_code=503,
        content={
            "error": "service_busy",
